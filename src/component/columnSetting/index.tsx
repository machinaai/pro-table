    ...
    <span className={`${className}-list-item`} key={columnKey}>
      ...
      <span className={`${className}-list-item-option`}>
        <ToolTipIcon
          columnKey={columnKey}
          fixed="left"
          title={intl.getMessage('tableToolBar.leftPin', 'Pin to left')}
          show={fixed !== 'left'}
        >
          <PushpinOutlined
            style={{
        ...
        <ToolTipIcon
          columnKey={columnKey}
          fixed={undefined}
          title={intl.getMessage('tableToolBar.noPin', 'Unpin')}
          show={!!fixed}
        >
        ...
        <ToolTipIcon
          columnKey={columnKey}
          fixed="right"
          title={intl.getMessage('tableToolBar.rightPin', 'Pin to right')}
          show={fixed !== 'right'}
        >
          <PushpinOutlined />
        </ToolTipIcon>
...
const GroupCheckboxList: React.FC<{
  ...
  return (
    <div className={`${className}-list`}>
      <CheckboxList
        title={intl.getMessage('tableToolBar.leftFixedTitle', 'Pinned to the left')}
        list={leftList}
        className={className}
      />
      {/* If there is nothing fixed, there is no need to display the title */}
      <CheckboxList
        list={list}
        title={intl.getMessage('tableToolBar.noFixedTitle', 'Not fixed')}
        showTitle={showLeft || showRight}
        className={className}
      />
      <CheckboxList
        title={intl.getMessage('tableToolBar.rightFixedTitle', 'Pinned to the right')}
        list={rightList}
        className={className}
      />
    </div>
    ...
    <ConfigConsumer>
      {({ getPrefixCls }: ConfigConsumerProps) => {
        ...
        return (
          ...
          >
            <Tooltip title={intl.getMessage('tableToolBar.columnSetting', 'Column settings')}>
              <SettingOutlined
